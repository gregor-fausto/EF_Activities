...
The best way to get a copy of this exercise, and all the other EcoForecast exercises, is to click on the project pull-down menu in the top-right corner of RStudio and select "New Project...".  
...
```
When the output of a function is not a number, R represents the output as `NaN`.

#### Question 2
```Why are we multiplying the results of the atan and atan2 functions by 180/pi? ```

The functions `atan` and `atan2` return values in radians. We multiply by `180/pi` to return values in degrees.

#### Question 3
```How is the atan2 function different from the atan function?```

The `atan` function takes one value: ... The `atan2` funtion takes two values: the (x,y) position of a point.

#### Question 4
```What is the difference between log and log10?```

The function `log` computes the natural logarithm (base e), while `log10` computes the common logarithm (base 10).

#### Question 5
```Given a right triangle with sides x = 5 and y = 13, calculate the length of the hypotenuse (show code)```
...
#### Question 6
```If a population starts with a density of 5 individuals/hectare and grows exponentially at a growth rate r=0.04/year, what is the size of the population in π years? (show code)```
...
```{r}
f = function(day,month,year){ 
  y = (year %% day) - month
  return(y)
}
f(day = 30, month = 3, year = 1991)
```
## R Scripts
